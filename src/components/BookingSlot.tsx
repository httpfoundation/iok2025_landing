...
export const BookingSlot = ({ member, userId, onClick }: BookingSlotProps) => {
  return (
    <Tooltip onClick={onClick} title={member ? member.name : 'Szabad hely'}>
      <Avatar
        variant="rounded"
        sx={{
          backgroundColor: getBookingSlotColor(member, userId),
          cursor: member && member.id.toString() !== userId.toString() ? 'not-allowed' : 'pointer',
        }}
      />
    </Tooltip>
  );
