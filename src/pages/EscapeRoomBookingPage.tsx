...
export const EscapeRoomBookingPage = () => {
  ...
  async function handleMemberChange(bookingId: number, slotIndex: number) {
    ...
    if (
      existing &&
      existing.members.findIndex((m) => m?.id.toString() === userId) === slotIndex &&
    ...
    <div>
      <Container maxWidth="sm" sx={{ py: 6 }}>
        <Typography variant="h4" fontWeight="bold">
          Szabadulószoba foglalás
        </Typography>
        <Typography variant="body1">
          Leírás a szabadulószoba foglaláshoz. Lorem impsum...
        </Typography>
        <Stack mt={3} gap={2}>
          <Grid spacing={2} alignItems="center" container>
            <Grid fontWeight="bold" item xs={3}>
              Dátum
            </Grid>
            <Grid fontWeight="bold" textAlign="center" item xs={4}>
              A csapat
            </Grid>
            <Grid item xs={1}></Grid>
            <Grid fontWeight="bold" textAlign="center" item xs={4}>
          ...
          {bookings?.map((booking) => (
            <Grid spacing={2} alignItems="center" container key={booking.id}>
              ...
              <BookingSlotList
                onSelect={(index) => handleMemberChange(booking.id, index)}
                userId={registration?.id ?? 0}
                members={booking.members.slice(0, 4)}
              />
              <Grid item xs={1} textAlign="left" />
              <BookingSlotList
                onSelect={(index) => handleMemberChange(booking.id, index + 4)}
                userId={registration?.id ?? 0}
                members={booking.members.slice(4, 8)}
              />
            </Grid>
          ))}
          <BookingInfo />
